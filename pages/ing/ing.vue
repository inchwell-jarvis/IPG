<template>
	<view class="box">
		...
		<!-- 基础信息 -->
		<view class="Customer">
			<view class="Customer_Top">暂停 / 结束</view>
			<div class="But_RTcjmi">
				<u-button v-if="JCData.State == 2 || JCData.State == 3 || JCData.State == 5" size="medium" type="warning" @tap="UpdateSRCStateInit(5)">暂停</u-button>
				<u-button v-if="JCData.State == 2 || JCData.State == 3 || JCData.State == 5" size="medium" type="error" @tap="UpdateSRCStateInit(4)">结束</u-button>

				<u-button v-if="!(JCData.State == 2 || JCData.State == 3 || JCData.State == 5)" size="medium" type="info">暂停</u-button>
				<u-button v-if="!(JCData.State == 2 || JCData.State == 3 || JCData.State == 5)" size="medium" type="info">结束</u-button>
			</div>
		</view>
		...
		<!-- 打卡信息 -->
		<view class="Customer">
			<view class="Customer_Top">打卡信息</view>

			<view class="Mini_list" style="margin: 10px 0">
				<view class="Mini_list_left">是否报销里程</view>
				<view class="Mini_list_right">
					<u-switch size="40" style="margin-top: 5px; float: left" v-model="checked" active-color="#19be6b" inactive-color="#fa3534" @change="switchB"></u-switch>
			...
			<view v-if="!checked" class="Mini_list" style="padding-bottom: 10px">
				<u-input v-model="numberPlate" type="text" :border="true" style="width: 70%; display: block; margin-left: 5%" placeholder="如使用公司车辆，请输入车牌号" />
				<u-button class="But" style="width: 20%; height: 34px" type="primary" @tap="Uploadlicense()">上传</u-button>
			</view>
		</view>
		...
		<view class="TimeLine">
			<view class="TimeLineLi">
				<view class="li" v-for="(item, index) in JCData.Details" :key="index">
					<h4>
						{{ item.StateStr + (item.State == 2 ? ' - ' + item.Len + ' km ' : '') }}
				...
				<view class="li">
					<h4>
						推荐总里程
						<span style="float: right">{{ (JCData.TotalTuiJianLen || '') + ' km ' }}</span>
					</h4>
					<h4>
						实际总里程
						<span style="float: right">{{ (JCData.TotalLen || '') + ' km ' }}</span>
					</h4>
				</view>
			</view>
		</view>
		...
		<view class="map">
			...
			<u-form v-if="JCData.State == 1">
				<text style="font-size: 13px">
					最佳里程
					<span style="margin-left: 45px">{{ BestMileage + 'km' }}</span>
					<u-button type="primary" size="mini" style="float: right" @click="BestMileageFUNCX()">重新获取</u-button>
				</text>
				...
				<u-form-item label-width="200" label="请选择">
					<u-radio-group v-model="radio" :wrap="true">
						<u-radio v-for="(item, index) in radioList" :key="index" :name="item.name" style="line-height: 40px" :disabled="item.disabled">
							{{ item.name }}
						</u-radio>
					</u-radio-group>
				</u-form-item>
				...
				<u-form>
					<u-form-item label-width="200" label="请选择">
						<u-radio-group v-model="CompleteState" :wrap="true">
							<u-radio v-for="(item, index) in CompleteList" :key="index" :name="item.name" style="line-height: 40px" :active-color="index == 0 ? 'red' : '#00ff00'" :disabled="item.disabled">
								{{ item.name }}
							</u-radio>
						</u-radio-group>
					</u-form-item>
			...
			<view class="mapManual" style="overflow: hidden; padding-right: 10px">
				<view class="Customer_Top">上传图片</view>
				<view style="width: 100%; min-height: 1px">
					<view v-for="(itemimg, index1) in JCData.Images" :key="index1" style="position: relative; width: 90px; height: 90px; float: left; margin: 5px">
						<u-icon name="close-circle-fill" color="#fa3534" size="35" style="position: absolute; top: 10px; right: 3px; z-index: 100" @tap="(delimgid = itemimg.ID), (DelImgshow = true)"></u-icon>

						<u-image class="img" width="90px" height="90px" :src="itemimg.Url" shape="square" border-radius="10" :fade="true" duration="450" @tap="lookimg(itemimg.Url)"></u-image>
					</view>
				</view>
				<u-upload width="90px" height="90px" style="float: left" ref="uUpload" :action="action" @on-choose-complete="submit()" :auto-upload="false"></u-upload>
			</view>
			...
			<view class="mapManual" style="padding-right: 10px">
				<view class="Customer_Top">
					备注
					<u-button @click="UpSaveMemo()" style="float: right" type="primary" size="medium" :loading="loadingState">上传</u-button>
				</view>
			...
			<!-- 出发 暂停 到达 -->
			<view class="botView">
				<u-modal v-model="botView_show" show-cancel-button @confirm="FunChoosing()" :content="botView_content"></u-modal>
				<u-button v-if="JCData.State == 2 || JCData.State == 3 || JCData.State == 5" type="primary" @tap="DepartureAndArrival(1)">出发</u-button>
				<u-button v-if="JCData.State == 1" type="primary" @tap="DepartureAndArrival(2)">到达</u-button>
			</view>
		</view>
				...
				.then((res) => {
					...
					uni.getLocation({
						...
						success: function (res) {
							...
							My.address = IP.province + IP.city + IP.district + IP.street + IP.streetNum + IP.poiName;
							// 如果已经手动定位 则不进入
